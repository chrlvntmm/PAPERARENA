...
export interface GameState {
  ...
  deathEventsThisTick: DeathEvent[];
}
...
export function tick(state: GameState) {
  ...
  for (const victimId of killed) {
    ...
    const cause = killedByOther ? 'killed' : 'self';
    state.deathEventsThisTick.push({
      victimId,
      cause,
      killerId: killedByOther && killerId !== undefined && killerId >= 0 ? killerId : null,
    });
    if (victim.isHuman && state.humanDeathCause === null) {
      state.humanDeathCause = cause;
    }
...
/** Force-eliminate a player (e.g. disconnect). Records a self-death event. */
export function forceEliminate(state: GameState, playerId: number) {
